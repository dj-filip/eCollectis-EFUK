import { useEffect, useState } from 'react'
import Box from '@mui/material/Box';
import Grid from '@mui/material/Grid';
import Button from '@mui/material/Button';
import Stack from '@mui/material/Stack';
import Typography from '@mui/material/Typography';
import TextField from '@mui/material/TextField';
import Pagination from '@mui/material/Pagination';
import TextareaAutosize from '@mui/material/TextareaAutosize';
import Divider from '@mui/material/Divider';
import Item from 'components/grid/item/item.components'
import { useNavigate } from 'react-router-dom';
import LocalizedDatePicker from 'components/localization/localized-date-picker.components';
import FormControl from '@mui/material/FormControl';
import InputLabel from '@mui/material/InputLabel';
import MenuItem from '@mui/material/MenuItem';
import Select from '@mui/material/Select';

import { getOrganizacioneJedinice } from 'services/fuk-organizaciona-jedinica.services'

const UpdateProcedure1 = (props) => {
  const { 
    organizacionaJedinicaName,
    setOrganizacionaJedinicaName,
    organizacionaJedinicaId,
    setOrganizacionaJedinicaId,
    sifraProcedure,
    setSifraProcedure,
    nazivProcedure, 
    setNazivProcedure,
    verzijaProcedure, 
    setVerzijaProcedure,
    rukovodilacOrganizacioneJedinice, 
    setRukovodilacOrganizacioneJedinice,
    nosilacPoslovnogProcesa, 
    setNosilacPoslovnogProcesa  
  } = props

  const [listOptionsIdAndName, setListOptionsIdAndName] = useState([]);
  

  useEffect(() => {
    (async () => {
      const res = await getOrganizacioneJedinice();
      
      let list = []
      for (let i = 0; i < res.data.data.length; i++) {
        list.push({ key: res.data.data[i].orgj_id, name: res.data.data[i].orgj_naziv })
      }

      setListOptionsIdAndName(list)
    })();
  }, []);

  return (
    <Grid container className="input-box" sx={{
      gap: "20px 40px",
    }}>
      {/* prvi red */}
      <Grid container sx={{
        "& > div": {
          width: "100%",
        },
      }}>  
        <TextField 
          fullWidth 
          id="procedura" 
          label="Процедура" 
          variant="outlined" 
          InputLabelProps={{ shrink: true }} 
          value={nazivProcedure}
          onChange={(e) => {
            setNazivProcedure(e.target.value)
          }} 
        />
      </Grid>
      {/* drugi red */}
      <Grid container className="half-wdth"  sx={{
        "& > div": {
          width: "100%",
        },
      }}>        
        <FormControl sx={{
          "& > div > div": {
            padding: "10px 0",
          },
        }}>
          <InputLabel id="dropdown-select-medium">Организациона јединица</InputLabel>
          <Select
            labelId="dropdown-select-medium"
            id="dropdown-select"
            InputLabelProps={{ shrink: true }} 
            value={organizacionaJedinicaName}
            label="Организациона јединица"
            onChange={(event) => {      
              const name = event.target.value;
              const element = listOptionsIdAndName.find((item) => {
                return item.name === name
              });
              const key = element.key
              if (key > 0) {
                setOrganizacionaJedinicaId(key)
                setOrganizacionaJedinicaName(name)
              }
            }}
          >
          <MenuItem key="0" value="Одаберите">
            <em>Одаберите</em>
          </MenuItem>
          {listOptionsIdAndName.map((item) => (
              <MenuItem key={item.key} value={item.name} >
                {item.name}
              </MenuItem>
            ))}
          </Select>
        </FormControl>     
      </Grid >
      <Grid container className="half-wdth">
      <TextField 
        fullWidth 
        id="sifraProcesa" 
        label="Шифра процедуре" 
        variant="outlined" 
        InputLabelProps={{ shrink: true }} 
        value={sifraProcedure}
        onChange={(e) => {
          setSifraProcedure(e.target.value)
        }}   
      />    
      </Grid>
      {/* treci red */}
      <Grid container className="half-wdth">
          <TextField 
            fullWidth 
            id="rukovodilacOrganizacioneJedinice" 
            label="Руководилац организационе јединице (власник процеса)" 
            variant="outlined"
            InputLabelProps={{ shrink: true }} 
            value={rukovodilacOrganizacioneJedinice} 
            onChange={(e) => {
              setRukovodilacOrganizacioneJedinice(e.target.value)
            }}  
          />    
      </Grid>
      <Grid container className="half-wdth">  
        <TextField 
          fullWidth 
          id="verzija" 
          label="Верзија" 
          variant="outlined"
          InputLabelProps={{ shrink: true }} 
          value={verzijaProcedure} 
          onChange={(e) => {
            setVerzijaProcedure(e.target.value)
          }}   
        />    
      </Grid>
      {/* cetrvri red */}
      <Grid container>  
          <TextField 
            fullWidth id="nazivProcedure" 
            label="Назив процедуре" 
            variant="outlined" 
            InputLabelProps={{ shrink: true }} 
            value={nazivProcedure} 
            onChange={(e) => {
              setNazivProcedure(e.target.value)
            }}   
          />    
      </Grid>
      {/* peti red */}
      <Grid container>  
          <TextField 
            fullWidth 
            id="nosilacPoslovnogProcesa" 
            label="Носилац пословног процеса (одговорно лице и корисник процеса)" 
            variant="outlined"
            InputLabelProps={{ shrink: true }} 
            value={nosilacPoslovnogProcesa} 
            onChange={(e) => {
              setNosilacPoslovnogProcesa(e.target.value)
            }}  
          />    
      </Grid>
    </Grid>
  )
}

export default UpdateProcedure1